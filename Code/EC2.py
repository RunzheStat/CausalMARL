...
# parallel = regions[25], reps[more.96], settings[pattern_seed, day]
...
##########################################################################################################################################################
shared_setting = "Basic setting:" + "[T, sd_O, sd_D, sd_R, sd_u_O, w_O, w_A, [M_in_R, mean_reversion, poisO, simple, u_O_u_D]] = " + str([T, sd_O, sd_D, sd_R, sd_u_O, w_O, w_A, DGP_choice]) + "\n"
...
for pattern_seed in [2]:
    for day in day_range:
        T = day * 48
        res = []
        print_setting = DASH + "[pattern_seed, day] = " + str([pattern_seed, day]) + "\n"
        print(print_setting)
        N_targets = simu(pattern_seed = pattern_seed, l = l, T = T, time_dependent = False, # Setting - general
                 ...
                 sd_D = sd_D, sd_R = sd_R, sd_O = sd_O, sd_u_O = sd_u_O, # Setting - noise
                  ...
                  CV_QV = True, penalty = [[1e-3, 1e-4], [1e-3, 1e-4]], 
