...
generateStateOfACity <- function(T=42, c="Tianjin"){
  ...
  if(province){
    ...
    removed <- (city_data$province_curedCount+city_data$province_deadCount)/(population)
  } else {
    infected <- city_data$city_confirmedCount/(population)
    removed <- (city_data$city_curedCount+city_data$city_deadCount)/(population)
  }

  suspected <- 1-infected-removed
  ...
  population <- rep(population,T)
  # include qianxi m and ratio r
  state <- data.frame(date=dates, suspected, infected, removed, action=city_action,
                      m19 =m_qianxi2019 , m20=m_qianxi2020, ratio=ratio19to20,
                      gdp,population=population, tl = t_prime_l )
  return(state)
}
