...
def MC_weight(l, u_O, time_dependent, mean_reversion, sd_D, sd_R, w_A, w_O, simple_grid_neigh, neigh, target_policy):
    # generate a long sample according to the beh 
#     tuples_i[t] = [S_it, A_it, R_it, Tsit, Tait, S_i(t+1), Tsi(t+1), pi_Sit_1, T_ait_1_pi]
    from sklearn.neighbors import KernelDensity
    T = 20000
    data, adj_mat, details = DG_once(seed = 1, l = l, T = T, 
                                     u_O = u_O, 
    ...
    data_target, adj_mat, details = DG_once(seed = 1, l = l, T = T, 
                                     u_O = u_O, 
                                     time_dependent = time_dependent, mean_reversion = mean_reversion, 
                                     OPE = True, target_policy = target_policy, 
                                     sd_D = sd_D, sd_R = sd_R, 
                                     w_A = w_A, w_O = w_O, 
                                     simple_grid_neigh = simple_grid_neigh)
    den_fun = []
    for i in range(l**2): 
        ...
        behav_states = arr([arr(tuples_i[t][0], tuples_i[t][3]) for t in range(T)])
        ...
        target_states = arr([arr(tuples_i[t][0], tuples_i[t][3]) for t in range(T)])
        
        
        den_fun_b = kde = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(behav_states)
#         np.exp(kde.score_samples(behav_states))
        den_fun_target = kde = KernelDensity(kernel='gaussian', bandwidth=0.2).fit(target_states)
        
        den_fun.append([den_fun_b, den_fun_target])
        
    return den_fun
    # generate a long sample according to the target
    # build model
    
    # calculate the ratio for the training dataset
...
def V_DR(data, pi, behav, adj_mat, Ts, Ta, penalty, dim_S_plus_Ts, n_cores = 10,
    ...
    den_fun = MC_weight(l = int(sqrt(N)), u_O, time_dependent, mean_reversion, sd_D, sd_R, w_A, w_O, simple_grid_neigh, neigh, target_policy)
    ...
    def getOneRegionValue(i):
        ...
        tuples_i = getRegionData(data[i], i, data_neigh, pi, Ts, Ta, mean_field = True, time_dependent = time_dependent)
        """ our method """
        Qi_diff, Vi = computeQV(tuples_i = tuples_i, R = R, dim_S_plus_Ts = dim_S_plus_Ts, 
                                CV_QV = CV_QV, penalty_range = penalty, print_flag = print_flag)
        if print_flag:
            print("QV time cost:", now() - a); a = now()
        wi = getWeight(tuples_i, policy0 = behav[i], policy1 = pi[i], dim_S_plus_Ts = dim_S_plus_Ts,
...
#         print(wi)
        mean_weight = np.mean(wi)
        if print_flag:
            print("WEIGHT time cost:", now() - a); a = now() # dominating part
        value_i_left = wi * (Ri + Qi_diff) / mean_weight
        DR_V = value_i_left 
#         - np.squeeze(arr(list(itertools.repeat(Vi, T))))
#         + Vi[0]
        if competing_methods: 
            """ move these methods out
            """
            # with MARL
            IS_V = wi * Ri / mean_weight
            
            Susan_V = Vi[0]
            
            DR2_V = IS_V + Susan_V - DR_V
            No_IS_V = np.mean(Ri)
            """ DR + w/o spatial """
            Qi_diff_NS, Vi_NS = computeQV(tuples_i = tuples_i, R = R, dim_S_plus_Ts = dim_S_plus_Ts, 
                                CV_QV = CV_QV, penalty_range = penalty, spatial = False, print_flag = print_flag)
            wi_NS = getWeight(tuples_i, policy0 = behav[i], policy1 = pi[i], dim_S_plus_Ts = dim_S_plus_Ts,
                              time_dependent = time_dependent, 
                          w_hidden = w_hidden, Learning_rate = Learning_rate,  n_layer = n_layer, 
                          batch_size = batch_size, max_iteration = max_iteration, test_num = test_num, epsilon = epsilon, 
                          spatial = False)
            DR_V_NS = wi_NS * (Ri + Qi_diff_NS) / np.mean(wi_NS) 
#             + Vi  - np.squeeze(arr(list(itertools.repeat(Vi_NS, T))))
            if print_flag:
                print("no_spatial:", now() - a); a = now()
...
#             return [np.mean(DR_V), np.mean(IS_V), Susan_V, np.mean(DR_V_NS), np.mean(DR_V_NMF)]
            ...
            return [np.mean(DR_V), np.mean(DR2_V), np.mean(IS_V), Susan_V, np.mean(DR_V_NS), No_IS_V]
        else:
            return np.mean(DR_V)
